...
const FALLBACK_URL =
  'https://raw.githubusercontent.com/zubiden/nulp-timetable-data/data/';
...
export async function getGroups(departmentparent_abbrname_selective = 'All') {
  return fetchHtml({ departmentparent_abbrname_selective })
    ...
    .catch((err) => {
      let fallback =
        FALLBACK_URL + `institutes/${departmentparent_abbrname_selective}.json`;
  ...
  return fetchHtml({
    ...
    .catch(async (err) => {
      const response = await fetch(
        FALLBACK_URL + `timetables/${studygroup_abbrname_selective}.json`
      );
      if (!response.ok) throw Error(err);
